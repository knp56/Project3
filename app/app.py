...
def api_add() -> str:
    data = request.json
    cursor = mysql.get_db().cursor
    inputData = (data['fldName'], data['fldLat'], data['fldLong'],
                 data['fldCountry'], data['fldAbbreviation'],
                 data['fldCapitalStatus'], data['fldPopulation'])
    sql_insert_query = """INSERT INTO tblCitiesImport (fldName,fldLat,fldLong,fldCountry,fldAbbreviation,fldCapitalStatus,fldPopulation) VALUES (%s, %s,%s, %s,%s, %s,%s) """
    cursor.execute(sql_insert_query, inputData)
    mysql.get_db().commit()
...
def api_edit(city_id) -> str:
    ...
    inputData = (data['fldName'], data['fldLat'], data['fldLong'],
                 data['fldCountry'], data['fldAbbreviation'],
                 data['fldCapitalStatus'], data['fldPopulation'], city_id)
    sql_update_query = """UPDATE tblCitiesImport t SET t.fldName = %s, t.fldLat = %s, t.fldLong = %s, t.fldCountry = %s, t.fldAbbreviation = %s, t.fldCapitalStatus = %s, t.fldPopulation = %s WHERE t.id = %s """
    cursor.execute(sql_update_query, inputData)
    mysql.get_db().commit()
